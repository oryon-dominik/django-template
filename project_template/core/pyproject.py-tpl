...
def load_pyproject(project_path: Path) -> PyProject:
    ...
    """
    try:
        with open(project_path / "pyproject.toml") as file:
            content = file.read()
    except FileNotFoundError:
        raise PyProjectTomlLoadException(f"pyproject.toml not found in {project_path}")
    except OSError as e:
        raise PyProjectTomlLoadException(f"error while reading pyproject.toml from {project_path}: {e}")
